...
/**
 * binary_trees_ancestor - this is  a function that finds the lowest common ancestor of
 * two nodes
				     ...
				     const binary_tree_t *second)
{
	...
	if (first == parent_second || parent_first == NULL ||
	   (parent_first->parent == NULL && parent_second != NULL))
	{
		lca = binary_trees_ancestor(first, parent_second);
		return (lca);
	}
	if (second == parent_first || parent_second == NULL ||
	   (parent_second->parent == NULL && parent_first != NULL))
	{
